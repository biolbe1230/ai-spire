...
public static class AIDecisionEngine
{
    ...
    public static async Task HandleMapChoiceCore(IRunState runState, Player player)
    {
        ...
        var decision = await GetDecisionInternal(state, PromptBuilder.BuildMapPrompt);
    ...
    /// <summary>
    /// 非战斗场景 LLM 调用（供 ScreenHandler 等外部调用）
    /// </summary>
    public static async Task<AIDecision> GetDecisionInternal(GameState state, Func<GameState, string> promptBuilder)
    {
        try
        {
        ...
        Log.Info("[AISpire] Using rule engine fallback (non-combat)");
        return state.Screen switch
        {
            ...
            _ => new AIDecision { Action = "end_turn", Reasoning = Loc.FallbackEndTurn }
        };
    }
    ...
    public static async Task HandleRoomEntered(AbstractRoom room, IRunState runState, Player player)
    {
        ...
        try
        {
            ...
            // Mirrors AutoSlayer: DrainOverlays → ClickProceed → HandleMap
        ...
        try
        {
            ...
            for (int round = 0; round < 10; round++)
            {
                ...
                if (buttons.Count == 0)
                {
                    ...
                    {
                        Log.Info("[AISpire] Clicking event proceed");
                ...
                var decision = await GetDecisionInternal(state, PromptBuilder.BuildEventPrompt);
                ...
                // If overlay opened, handle all overlay screens (upgrade, card selection, rewards, etc.)
    ...
    public static async Task HandleRestSite(RestSiteRoom restRoom, IRunState runState, Player player)
    {
        ...
        try
        {
            ...
            // Find enabled rest site buttons
            ...
            // Extract state and ask LLM
            var state = GameStateExtractor.ExtractRestSiteState(restRoom, runState, player);
            AIOverlay.SetStatus(Loc.AnalyzingCamp);
            var decision = await GetDecisionInternal(state, PromptBuilder.BuildRestSitePrompt);

            var idx = decision.EventOptionIndex;
            if (idx < 0 || idx >= restBtns.Count) idx = 0;

            var optName = restBtns[idx].Name;
            Log.Info($"[AISpire] Camp decision: {optName} (idx:{idx}) | reason: {decision.Reasoning}");
            AIOverlay.AddMessage(Loc.Camp, $"{optName}: {decision.Reasoning}");

            await UiHelper.Click(restBtns[idx]);
            await Task.Delay(1500);
            ...
            if (overlayCheck != null && overlayCheck.ScreenCount > 0)
            {
                ...
                await ScreenHandler.DrainOverlayScreens(runState, player);
                // After overlay, wait for proceed button
    ...
    public static async Task HandleShop(MerchantRoom shopRoom, IRunState runState, Player player)
    {
        ...
        try
        {
            ...
            for (int shopRound = 0; shopRound < 10; shopRound++)
            {
                ...
                // Check if affordable items exist
                var gold = state.Player?.Gold ?? 0;
                if (!state.Shop!.Items.Any(i => i.Price <= gold))
                {
                ...
                var decision = await GetDecisionInternal(state, PromptBuilder.BuildShopPrompt);
                ...
                if (itemIdx < 0 || itemIdx >= state.Shop.Items.Count)
                {
                    Log.Info($"[AISpire] Shop decision: invalid index {itemIdx}");
                    break;
                }

                var item = state.Shop.Items[itemIdx];
                if (item.Price > gold)
                {
                ...
                var purchaseOk = ActionExecutor.ExecuteShopPurchase(itemIdx, runState, player);
                if (!purchaseOk)
                {
    ...
    public static async Task HandleTreasure(IRunState runState, Player player)
    {
        ...
        try
        {
            ...
            // 3.5) Drain overlay screens (e.g. relic AfterObtained triggers upgrade screen)
            await ScreenHandler.DrainOverlayScreens(runState, player);

            // 4) 等待 ProceedButton 启用并点击
    ...
    public static async Task HandleCardReward(IReadOnlyList<CardInfo> cardChoices, IRunState runState, Player player)
    {
        if (_isProcessing) return;
        _isProcessing = true;
        ...
        try
        {
            ...
            var state = new GameState
            {
                ...
                CardChoices = cardChoices.ToList()
            };

            AIOverlay.SetStatus(Loc.AnalyzingCards);
            var decision = await GetDecisionInternal(state, PromptBuilder.BuildCardSelectionPrompt);

            if (decision.Action == "skip_reward")
            {
                Log.Info($"[AISpire] Card reward: skip | reason: {decision.Reasoning}");
                AIOverlay.AddMessage(Loc.CardReward, string.Format(Loc.SkipFmt, decision.Reasoning));
            }
            else
            {
                var idx = decision.CardChoiceIndex;
                if (idx < 0 || idx >= cardChoices.Count) idx = 0;
                var cardName = cardChoices[idx].Name;
                Log.Info($"[AISpire] Card reward: choose[{idx}] {cardName} | reason: {decision.Reasoning}");
        ...
        {
            Log.Info($"[AISpire] HandleCardReward error: {e.Message}");
        }
        finally
        {
            _isProcessing = false;
